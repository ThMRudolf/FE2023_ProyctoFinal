...
ya que es una constant en ambas $Log()$.

```{r}
log_verosim_multinominal <- function(p_vec, n_vec, n){
 lvmulti <- n_vec[1]*log(p_vec[1])
 if (n>1)
 {
   for (idx in 2:n)
   {
     lvmulti <- n_vec[idx]*log(p_vec[idx]) + lvmulti
   }
 }
}
lv_multi_hat  <- x[1]*log(p_hat[1]) + x[2]*log(p_hat[2]) + 
   x[3]*log(p_hat[3]) + x[4]*log(p_hat[4])
 
lv_multi_0 <- x[1]*log(p0[1]) + x[2]*log(p0[2]) + 
   x[3]*log(p_hat[3]) + x[4]*log(p0[4])
 
lambda_multinominal <- 2*(lv_multi_hat - lv_multi_0)
 result_lambda %>% mutate(lambda_mn =lambda_multinominal) %>% gt()
```
...
sigma_init <- 1/sqrt(dist_inicial_tau)
# graficar para buscar los valores de a y b
g_dist_inicial <- ggplot(tibble(sigma_init), aes(sigma_init)) + geom_histogram() + labs(title="histogram to find a and b\n as close as possible to \n the prametric bootstrap")
density_gamma <-tibble(x = 1:n_samples, p=sigma_init)
...
varianza no tan alta. Con eso llegamos a a=2 y b = 100.
...
dist_post_sigma <- sqrt(1/rgamma(n_samples, a_post, b_post))
ee_dist_post_sigma <- sd(dist_post_sigma)
mean_dist_post_sigma <- mean(dist_post_sigma)
dist_post_sigma_tbl <- tibble(dist_post = dist_post_sigma)
ggplot(dist_post_sigma_tbl, aes(dist_post)) + geom_histogram()

dist_post_sigma_tbl_sum <- dist_post_sigma_tbl %>% summarise(ee_dist = sd(dist_post), 
                                  mean_dist = mean(dist_post))
(dist_post_sigma_tbl_sum) %>% gt()
```
...
crear_verosim_tau <- function(sigma){
  log_p <- function(pars){
    a = pars[1]
    b = pars[2]
    # 
    verosim <- -(a+1)*log(sigma) - b/sigma - log(gamma(a)) -a*log(b)
  }  
  log_p
}
n_samples <- 150
sample_invgamma <- sqrt(1/rgamma(n_samples, a, b))

verosim_tau <- crear_verosim_tau(sample_invgamma)

res <- optim(c(2, 100), verosim_tau, control = list(fnscale = -1), method = "Nelder-Mead")
res$convergence

a_init <- res$par[1]
b_init <- res$par[2]

simular_modelo <- function(n, a, b){
  sigma <- sqrt(1/rgamma(n,a, b))
}

rep_boot <- function(rep, crear_verosim_tau, tau_init, n_samples){
  muestra_bootstrap <- simular_modelo(n_samples, a,b)
  log_p_boot <- crear_verosim_tau(a, b) #crear_verosim_tau(muestra_bootstrap) 
  # optimizamos
  res_boot <- optim(c(0.5),crear_verosim_tau, lower=0, upper=200,
    control = list(fnscale = -1, maxit = 1000), method = "Brent")
  try(if(res_boot$convergence != 0) stop("No se alcanzó convergencia."))
  tibble(parametro = c("sigma"), estimador_boot = res_boot$par) 
}
reps_boot <- map_dfr(1:5000, ~ rep_boot(.x, crear_verosim_tau, 
                                        tau_init, n_samples)) 
reps_boot
```
...
# compilar
modelo_stan  <- cmdstan_model(archivo_stan)
# Ajustar el modelo
#ajuste <- sampling(modelo_stan, data = datos, chains = 4, iter = 2000)
ajuste <- modelo_stan$sample(data = datos, 
                             seed = 1234,
                             chains = 4,
                             refresh = 2000)
