...
est_mle
n_sample <- length(x)
sim_distr <- rnorm(n_sample, mean=0, sd=res$par)
...
```{r}
rep_boot <- function(rep, crear_log_sigma, est_mle, n){
  muestra_bootstrap <- simular_modelo(n, 
                               est_mle["estimated sigma", "estimador"])
  log_p_boot <- crear_log_sigma(muestra_bootstrap)
  # optimizamos
  res_boot <- optim(100, log_p_boot, lower = 0.001, upper = 100,
  control = list(fnscale = -1, maxit = 1000), method = "Brent")
...
reps_boot <- map_dfr(1:5000, ~ rep_boot(.x, crear_log_sigma, est_mle, 
                                        n = n_sample, rep = ".id")) 
reps_boot
```
