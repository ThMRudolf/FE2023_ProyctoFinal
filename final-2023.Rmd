...
# quitar los estados Alaska, Hawai, DC
poll_data_2eval <- poll_data %>% filter(state != 'alaska' & state != 'hawaii' & state != 'dc')

# prepare % per state, el valor de prop_vl es la max. versosimilitud
poll_data_2eval_by_state <- poll_data_2eval %>% group_by(state) %>% summarise(prop_vl = mean(ideo == "very liberal", na.rm=TRUE), n = sum(!is.na(ideo)), k = sum(ideo == "very liberal", na.rm=TRUE))
poll_data_2eval_by_state %>% gt()

# Graficar con ggplot2
ggplot(poll_data_2eval_by_state, aes(x = n, y = prop_vl)) +
  geom_point(aes(color = state) )+
  labs(x = "numero de preguntados", y = "procentaje de 'very liberal'" ) 
```
...
```{r}
# read 
# Obama era el candidato democrata --> votos para "democrat" don votos para Obama
data2008 <- read.csv("data/2008ElectionResult.csv")

data2008_2eval <- data2008 %>%  filter(state != 'Alaska' & state != 'Hawaii' & state != 'DC')
plot_data <- tibble(x_very_liberal = poll_data_2eval_by_state$prop_vl*100, 
                    votes_4_Obama = data2008_2eval$vote_Obama_pct,
                    states = data2008_2eval$state) 

ggplot(plot_data, aes(x = x_very_liberal, y = votes_4_Obama)) +
   geom_point(aes(color = states) ) +
  labs(x = "% de 'very liberal'", y= "% votos para Obama" ) 
