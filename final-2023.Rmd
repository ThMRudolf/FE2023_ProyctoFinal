...
plot_data <- bind_rows(x_samples, x_sample_opt_sigma)
...
a <- 550
b <- 50
dist_inicial <- rgamma(n_samples, a, b)
# graficar para buscar los valores de a y b
g_dist_inicial <- ggplot(tibble(dist_inicial), aes(dist_inicial)) + geom_histogram()
g_dist_inicial
density_gamma <-tibble(x = 1:n_samples, p=dist_inicial)
plot_g <- ggplot(density_gamma, aes(x=p)) + geom_density(fill = "skyblue", color= "black", alpha = 0.7)+
  labs(title ="inicial pdf of selected inverse gamma distribution, a = 550, b=50")
plot_g

```

Con trial & error buscamos un valor que tiene un pico en un valor aprox
de la 11 (bootstrap parametrico nos dio un valor de 11.69) y con una
varianza no tan alta. Con eso llegamos a a = 550 y b = 50.
